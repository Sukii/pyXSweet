...
<xsl:stylesheet
  ...
  <!-- Remove any inline formatting on only whitespace. -->
  <xsl:template match="p//*[not(matches(.,'\S'))]">
    <xsl:apply-templates/>
  </xsl:template>
  ...
  <!-- copy everything in the ooxml math namespace -->
  <xsl:template xmlns:oom="http://schemas.openxmlformats.org/officeDocument/2006/math"
    match="oom:*" priority="10">
    <xsl:copy>
      <xsl:apply-templates select="node() | @*"/>
    </xsl:copy>
  </xsl:template>
