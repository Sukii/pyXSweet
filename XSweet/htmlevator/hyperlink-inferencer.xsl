...
<!--  The rationale of the logic is as follows: uri-match is fairly greedy, accepting anything that looks like it
  *might* be (or have) a URI *after the text has been split around white space*. This means that our regex can
  be loose - because we back it up by using the XPath 'castable as' operator to screen out the non-URIs from
  the URIs. Under testing this appears to work surprisingly well. -->
